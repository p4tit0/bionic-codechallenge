...
const filters = ref({ type: null, deviceType: null });
...
async function loadAlarms() {
  const data = await fetchAlarms({ page: page.value, limit: limit.value, type: filters.value.type, deviceType: filters.value.deviceType });
...
onMounted(async () => {
  ...
  socket.on('newAlarm', (alarm: Alarm) => {
    ...
    if ((f.type && f.type !== alarm.type) || (f.deviceType && f.deviceType !== alarm.deviceType)) {
      total.value += 1;
      return;
    }
...
<template>
  <div style="max-width:1000px;margin:0 auto;padding:16px">
    ...
    <AlarmList :alarms="alarms" :total="total" :page="page" :limit="limit" @page-change="onPageChange" />
  </div>
</template>
